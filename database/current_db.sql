...
CREATE TABLE public.breach_updates (
  ...
  update_type text CHECK (update_type = ANY (ARRAY['discovery'::text, 'new_info'::text, 'class_action'::text, 'fine'::text, 'remediation'::text, 'resolution'::text])),
...
CREATE TABLE public.breaches (
  ...
  breach_method text,
  attack_vector text CHECK (attack_vector = ANY (ARRAY['phishing'::text, 'ransomware'::text, 'api_exploit'::text, 'insider'::text, 'supply_chain'::text, 'misconfiguration'::text, 'other'::text])),
